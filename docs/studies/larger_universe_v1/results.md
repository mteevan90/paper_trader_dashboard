...
> **Correction (2026-05-13):** The +0.048 top-quintile IC cited above was computed with `prices` restricted to v1's held tickers (~450 across XGB + ENet), not the full eligible universe. Under the standard full-cross-section definition (~1,963 tickers), the XGBoost top-quintile IC is **−0.004**, not +0.048 — sign reversal. The finding "full-cross-section IC was the wrong CV objective" rests on a logical-structure argument (operating region matters) that is independent of v1's specific IC values, but the empirical evidence cited here is not what it appears to be. The architectural memo's correction section explains the implications and explicitly does NOT modify the memo's recommendation; re-justification of the recommendation is an open workstream. Full audit: [`docs/studies/larger_universe_v1/ic_scope_audit.md`](ic_scope_audit.md).
...
> **Correction (2026-05-13):** The contrast in this paragraph reads as if both numbers are computed at the same scope. They are not. The −0.009 full-cross-section IC was indeed full-cross-section (the formula's full-IC computation is robust at any scope to FP precision; v2 reproduces it bit-for-bit). The +0.048 top-quintile IC was computed with `prices` restricted to v1's held tickers (~450), not the full eligible universe (~1,963). Under matching full-cross-section scope, the XGBoost top-quintile IC is **−0.004**, not +0.048. The "model has real skill where the strategy uses it" claim does not survive the scope correction at the empirical level. Full audit: [`ic_scope_audit.md`](ic_scope_audit.md). The architectural memo's correction section explains why this changes the empirical foundation of the methodology finding without (yet) changing the recommendation, which rests on a separate logical-structure argument.
